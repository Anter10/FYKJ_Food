...
/**
 * Created by guoyouchao on 16/7/11.
 */
public class FoodsCell extends BaseAdapter{


    private List<Map<String, FoodDataModel>> tablesData;
    private LayoutInflater layoutInflater;
    private Context context;

    public FoodsCell(Context context, List<Map<String, FoodDataModel>> data){
    ...
    public final class FoodDataView{
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        FoodDataView tmv = null;
        if(convertView==null){
            tmv = new FoodDataView();
            //获得组件，实例化组件
            convertView=layoutInflater.inflate(R.layout.foodsinglexml, null);

            convertView.setTag(tmv);
        }else{
            tmv=(FoodDataView)convertView.getTag();
        }
        //绑定数据
